...
class BILLCLASS:
    def __init__(self,root):
        self.root=root
        self.root.geometry("1360x640+0+0")
        self.root.title("Inventory Management System | Developed by Jinde Anantha Sai")
        self.root.config(bg="white")
        self.cart_list=[]
        self.chk_print=0

        #==title===
        self.icon_title=PhotoImage(file="assets/images/logo1.png")
        ...
        btn_search=Button(productFrame2,text="Search",command=self.search,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=290,y=45,width=100,height=26)
        btn_show_all=Button(productFrame2,text="Show All",command=self.show,font=("goudy old style",15),bg="#083531",fg="white",cursor="hand2").place(x=290,y=10,width=100,height=26)

        #======Product Details Frame===========
        productframe3=Frame(productFrame1,bd=3,relief=RIDGE)
        productframe3.place(x=2,y=130,width=398,height=355)
        ...
        self.product_Table.config(xscrollcommand=scrollx.set,yscrollcommand=scrolly.set)

        self.product_Table.heading("pid",text="Prod_ID")
    ...
    #==============CustomerFrame============
        ...
        cTitle=Label(CustomerFrame,text="Customer Details",font=("goudy old style",18),bg="lightgrey").pack(side=TOP,fill=X)
        lbl_name=Label(CustomerFrame,text="Name",font=("times new roman",15),bg="white").place(x=5,y=35)
        txt_name=Entry(CustomerFrame,textvariable=self.var_cname,font=("times new roman",15),bg="lightyellow").place(x=80,y=35,width=140)
        ...
        Cal_cart_Frame.place(x=420,y=185,width=530,height=340)

        #===Calculator Frame=========
        self.var_cal_input=StringVar()
        Cal_Frame=Frame(Cal_cart_Frame,bd=9,relief=RIDGE,bg="white")
        Cal_Frame.place(x=5,y=10,width=268,height=320)
        ...
        cart_Frame.place(x=280,y=8,width=245,height=320)
        ...
        self.carttable.config(xscrollcommand=scrollx.set,yscrollcommand=scrolly.set)
        ...
        self.carttable.column("name",width=20)
        self.carttable.column("price",width=20)
        ...
        Add_CartWidgetFrame.place(x=420,y=525,width=530,height=105)
        ...
        txt_p_qty=Entry(Add_CartWidgetFrame,textvariable=self.var_qty,font=("times new roman",15),bg="lightyellow").place(x=390,y=35,width=130,height=22)
    ...
    #===Billing Area========================
        ...
        billFrame.place(x=952,y=110,width=320,height=360)
    ...
    #==========Billing Buttons==================
        ...
        billMenuFrame.place(x=953,y=470,width=320,height=160)
        ...
        self.lbl_net_pay.place(x=218,y=5,width=98,height=70)
        ...
        btn_print.place(x=2,y=80,width=105,height=70)
        ...
        btn_clear_all.place(x=110,y=80,width=105,height=70)


        btn_generate=Button(billMenuFrame,text='Generate\n/ Save Bill',command=self.generate_bill,cursor='hand2',font=('goudy old style',13,"bold"),bg="#009688",fg="white")
        btn_generate.place(x=218,y=80,width=98,height=70)
...
#=========================All functions========================
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                ...
                cur.execute("select pid,name,price,qty,status from product where name LIKE ? and status='Active'",(f"%{self.var_search.get()}%",))
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.product_Table.delete(*self.product_Table.get_children())
                    for row in rows:
    ...
    def add_update_cart(self):
        if self.var_pid.get()=='':
            ...
            messagebox.showerror('Error',"Quantity should be numeric",parent=self.root)
    ...
    def generate_bill(self):
        if self.var_cname.get()=="" or self.var_contact.get()=="":
            ...
            #=======bill bottom==
            self.bill_bottom()
            with open(f'data/bills/{str(self.invoice)}.txt','w') as fp:
                fp.write(self.txt_bill_area.get('1.0',END))
            messagebox.showinfo('Saved',"Bill has been generated/Save in the Backend",parent=self.root)
            self.chk_print=1
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t    XYZ-Inventory
Phone No. 9515432346 , Delhi-125001
{str("="*36)}
